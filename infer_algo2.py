...
infer_algo2.py
========================================================================
Inference for BL-LCILW Algorithm 2 with the folded-frequency x-update.

Pipeline (boundary-safe):

    HR -> reflect pad -> degrade -> ADMM/PnP -> crop HR pad -> metric/save

Padding the HR image before degradation (rather than padding the LR after
degradation) removes black edge/rim artifacts; the pad is cropped away
before PSNR computation and saving.

RGDN loading is robust: the architecture (e.g. 64/8 or 128/12) is inferred
from the checkpoint state_dict when possible, and torch.compile
'_orig_mod.' prefixes are stripped if present.
...
def fft_solve_balanced(otf, scale, mu, rho, rhs):
    """Solve (H^T D^T D H + (mu+rho)I)x = rhs via frequency folding."""
    B, C, H, W = rhs.shape
    lam = mu + rho
...
def strip_compile_prefix(state_dict):
    """Remove torch.compile '_orig_mod.' prefix if present."""
...
def infer_rgdn_arch_from_state(state_dict):
    """Infer RGDN(num_features, num_blocks) from a state_dict."""
...
def build_rgdn_from_checkpoint_dict(ckpt_obj, device):
    """Build RGDN from a checkpoint object or raw state_dict."""
...
def make_denoiser(net_R: nn.Module, r: float):
    """Averaged operator used in the lower level. MUST match training relaxation."""
    if r >= 1.0:
...
def super_resolve(y, net_W, net_A, denoiser, mu, rho, scale, L, kernel_base):
    """PnP-ADMM forward on the given LR image. Returns sr_x, U_w_y, alpha."""
    U_w_y = net_W(y)
    alpha_map = net_A(y)
...
def main():
    ...
    for p in paths:
        ...
        # Boundary fix: pad HR before degradation, not LR after degradation.
        ...
        # Crop away HR padding before PSNR/saving.
